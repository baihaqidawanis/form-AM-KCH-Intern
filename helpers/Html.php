...
?>
	...
	public static function render_submenu($arrMenu, $menu_type = "dropdown", $parent_id = null, $group_id = null)
	{
		...
		if (!empty($arrMenu)) {
			// Keep the current menu group open after navigation. Without this, users
			// have to reopen Filling every time they switch between SIG and Joeya.
			$expanded_class = "";
			$current_path = trim(urldecode($page_url), "/");
			foreach ($arrMenu as $menuobj) {
				$submenu_path = trim((string) parse_url($menuobj['path'], PHP_URL_PATH), "/");
				if ($submenu_path !== "" && ($page_name == $submenu_path || $current_path == $submenu_path || strpos($current_path, $submenu_path . "/") === 0)) {
					$expanded_class = " show";
					break;
				}
			}
		?>
			<ul id="<?php echo $parent_id; ?>" class="<?php echo $menu_class . $expanded_class ?> submenu list-unstyled" <?php echo $data_parent; ?>>
				<?php
