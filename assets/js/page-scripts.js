...
$('form').submit(function (event) {
    ...
    // Trigger HTML5 form validation manually
    if (!form[0].checkValidity()) {
        // If form is invalid, re-enable the submit button and prevent submission
        submitButton.prop('disabled', false);
        event.preventDefault(); // Prevent form submission

        // You can manually trigger the browser's validation UI (red borders, etc.)
        form[0].reportValidity(); // This triggers the browser's default validation UI
        return false;
    }

    // If the form is valid, allow submission to proceed
    // The button will remain disabled during processing
});

document.addEventListener("DOMContentLoaded", function () {
    document.querySelectorAll(".guide-image").forEach(function (element) {
        let img = document.createElement("img");
        img.src = "/form-am/assets/images/E-AM.png"; 
        img.alt = "Guide Image";
