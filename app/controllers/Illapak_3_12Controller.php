...
class Illapak_3_12Controller extends SecureController
{
	private $parts = array(
	...
	function __construct() { parent::__construct(); $this->tablename = 'illapak_3_12'; }
	...
	{
		$request = $this->request; $db = $this->GetModel(); $fields = array('illapak_3_12.id_illapak_3_12', 'mesin.nama_mesin AS nm_mesin', 'illapak_3_12.created_at', 'illapak_3_12.user_create', 'illapak_3_12.user_approve', 'illapak_3_12.approval', 'illapak_3_12.tanggal_perubahan');
		foreach ($this->part_fields() as $part) { $fields[] = 'illapak_3_12.' . $part; }
		if (!empty($request->search)) {
			$like = '%' . trim($request->search) . '%';
			$search_fields = array_merge(array('mesin.nama_mesin', 'illapak_3_12.user_create', 'illapak_3_12.user_approve', 'illapak_3_12.approval', 'illapak_3_12.kendala'), array_map(function($part) { return 'illapak_3_12.' . $part; }, $this->part_fields()));
		...
		if (!empty($request->date_from)) { $db->where('illapak_3_12.created_at', trim($request->date_from) . ' 00:00:00', '>='); }
		if (!empty($request->date_to)) { $db->where('illapak_3_12.created_at', trim($request->date_to) . ' 23:59:59', '<='); }
		if (!empty($request->mesin)) { $db->where('illapak_3_12.mesin', $request->mesin); }
		if ($fieldname) { $db->where($fieldname, $fieldvalue); }
		$db->join('mesin', 'illapak_3_12.mesin = mesin.id', 'LEFT')->orderBy('illapak_3_12.id_illapak_3_12', ORDER_TYPE);
		...
		$this->view->page_title = 'Illapak 3 - 12'; $this->set_report_props('Illapak 3 - 12', 'landscape');
		return $this->render_view('illapak_3_12/list2.php', $this->page_data($records, $tc));
	}
				...
				if ($rec_id) {
					foreach ($this->parts as $field => $label) {
						if (!empty($_POST['kendala_' . $field])) {
							$db->insert('kendala_illapak_3_12', array('id_am' => $rec_id, 'mesin' => $modeldata['mesin'], 'nama_bagian' => $field, 'kendala' => $_POST['kendala_' . $field], 'kategori_tag' => $_POST['kategori_tag_' . $field], 'korelasi_tag' => $_POST['korelasi_tag_' . $field], 'klasifikasi_tag' => $_POST['klasifikasi_tag_' . $field], 'kategori_ketidaksesuaian' => $_POST['kategori_ketidaksesuaian_' . $field], 'created_at' => datetime_now()));
						}
					}
					$this->write_to_log('add', 'true'); $this->set_flash_msg('Berhasil tambah AM Illapak 3-12', 'success'); return $this->redirect('illapak_3_12');
				}
				$this->set_page_error();
			}
		}
		$this->view->page_title = 'Add New AM Illapak 3-12'; return $this->render_view('illapak_3_12/add.php', array('parts' => $this->parts));
	}

	function view($rec_id = null, $value = null)
	{
		$db = $this->GetModel(); $this->rec_id = $rec_id; $fields = array_merge(array('illapak_3_12.id_illapak_3_12', 'illapak_3_12.mesin', 'mesin.nama_mesin AS nm_mesin', 'illapak_3_12.created_at', 'illapak_3_12.user_create', 'illapak_3_12.user_approve', 'illapak_3_12.approval', 'illapak_3_12.tanggal_perubahan'), array_map(function($part) { return 'illapak_3_12.' . $part; }, $this->part_fields()));
		if ($value) { $db->where($rec_id, urldecode($value)); } else { $db->where('illapak_3_12.id_illapak_3_12', urldecode($rec_id)); }
		$record = $db->join('mesin', 'illapak_3_12.mesin = mesin.id', 'LEFT')->getOne($this->tablename, $fields);
		if ($record) {
			$details = $db->rawQuery('SELECT k.*, t1.kategori_tag AS teks_kategori, t2.nama AS teks_korelasi, t3.nama AS teks_klasifikasi, t4.kategori AS teks_ketidaksesuaian FROM kendala_illapak_3_12 k LEFT JOIN tag t1 ON k.kategori_tag=t1.id LEFT JOIN korelasi t2 ON k.korelasi_tag=t2.id LEFT JOIN klasifikasi t3 ON k.klasifikasi_tag=t3.id LEFT JOIN kategori t4 ON k.kategori_ketidaksesuaian=t4.id WHERE k.id_am=?', array($record['id_illapak_3_12']));
		...
		$record['parts'] = $this->parts; $this->view->page_title = 'View AM Illapak 3-12'; $this->set_report_props('View AM Illapak 3-12');
		return $this->render_view('illapak_3_12/view.php', $record);
	}
	...
	function edit($rec_id = null, $formdata = null)
	{
		...
		if ($formdata) {
			...
			if ($this->validated()) {
				$db->where('id_illapak_3_12', $rec_id);
				$bool = $db->update($this->tablename, $modeldata);
				$numRows = $db->getRowCount();
				if ($bool && $numRows) { $this->write_to_log('edit', 'true'); $this->set_flash_msg('Approval berhasil diperbarui', 'success'); return $this->redirect('illapak_3_12'); }
				if ($db->getLastError()) {
					$this->set_page_error();
				} elseif (!$numRows) {
					$page_error = 'No record updated';
					$this->set_page_error($page_error); $this->set_flash_msg($page_error, 'warning'); return $this->redirect('illapak_3_12');
				}
			}
		}
		$db->where('id_illapak_3_12', $rec_id); $data = $db->getOne($this->tablename, array('id_illapak_3_12', 'approval'));
		if (!$data) { $this->set_page_error('No record found'); }
		$this->view->page_title = 'Edit Approval AM Illapak 3-12'; return $this->render_view('illapak_3_12/edit.php', $data);
	}
		...
		if ($formdata) {
			...
			if ($this->validated()) {
				...
				if ($bool) {
					...
					$db->delete('kendala_illapak_3_12');
					foreach ($this->parts as $field => $label) {
						if (!empty($_POST['kendala_' . $field])) {
							$db->insert('kendala_illapak_3_12', array('id_am' => $rec_id, 'mesin' => $mesin_id, 'nama_bagian' => $field, 'kendala' => $_POST['kendala_' . $field], 'kategori_tag' => $_POST['kategori_tag_' . $field], 'korelasi_tag' => $_POST['korelasi_tag_' . $field], 'klasifikasi_tag' => $_POST['klasifikasi_tag_' . $field], 'kategori_ketidaksesuaian' => $_POST['kategori_ketidaksesuaian_' . $field], 'created_at' => datetime_now()));
						}
					}
					$this->write_to_log('edit_data', 'true');
					$this->set_flash_msg('Data berhasil diperbarui', 'success');
					return $this->redirect('illapak_3_12/view/' . $rec_id);
				}
				if ($db->getLastError()) {
					$this->set_page_error();
				} elseif (!$numRows) {
					$page_error = 'Tidak ada perubahan data yang disimpan';
					$this->set_page_error($page_error); $this->set_flash_msg($page_error, 'warning'); return $this->redirect('illapak_3_12/view/' . $rec_id);
				}
			}
		}
		$db->where('illapak_3_12.id_illapak_3_12', $rec_id);
		$record = $db->join('mesin', 'illapak_3_12.mesin = mesin.id', 'LEFT')->getOne($this->tablename, array('illapak_3_12.*', 'mesin.nama_mesin AS nm_mesin'));
		if ($record) {
			$details = $db->rawQuery('SELECT k.* FROM kendala_illapak_3_12 k WHERE k.id_am=?', array($rec_id));
			$record['abnormalitas'] = array(); foreach ($details as $detail) { $record['abnormalitas'][$detail['nama_bagian']] = $detail; }
		} else {
			$this->set_page_error($db->getLastError() ?: 'No record found'); $record = array();
		}
		$record['parts'] = $this->parts; $this->view->page_title = 'Edit Data AM Illapak 3-12';
		return $this->render_view('illapak_3_12/edit_data.php', $record);
	}

	function editfield($rec_id = null, $formdata = null)
	{
		$db = $this->GetModel(); $this->rec_id = $rec_id;
		$this->fields = array('id_illapak_3_12', 'updated_at', 'approval', 'user_approve', 'perubahan', 'user_perubah', 'tanggal_perubahan');
		$page_error = null;
		if ($formdata) {
			$postdata = array();
	...
	function delete($rec_id = null) { Csrf::cross_check(); $db = $this->GetModel(); $this->rec_id = $rec_id; $ids = array_map('trim', explode(',', $rec_id)); $db->where('id_illapak_3_12', $ids, 'in'); if ($db->delete($this->tablename)) { $this->write_to_log('delete', 'true'); $this->set_flash_msg('Record deleted successfully', 'success'); } else { $this->set_flash_msg($db->getLastError(), 'danger'); } return $this->redirect('illapak_3_12'); }
}
