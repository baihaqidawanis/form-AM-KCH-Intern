<?php
/** Autonomous Maintenance Unifill B (Filling). */
class Unifill_bController extends SecureController
{
	private $parts = array(
		'conveyor' => 'Conveyor',
	...
	function __construct() { parent::__construct(); $this->tablename = 'unifill_b'; }
	...
	{
		$request = $this->request; $db = $this->GetModel(); $fields = array('unifill_b.id_unifill_b', 'mesin.nama_mesin AS nm_mesin', 'unifill_b.created_at', 'unifill_b.user_create', 'unifill_b.user_approve', 'unifill_b.approval', 'unifill_b.tanggal_perubahan');
		foreach ($this->part_fields() as $part) { $fields[] = 'unifill_b.' . $part; }
		if (!empty($request->search)) {
			$like = '%' . trim($request->search) . '%';
			$search_fields = array_merge(array('mesin.nama_mesin', 'unifill_b.user_create', 'unifill_b.user_approve', 'unifill_b.approval', 'unifill_b.kendala'), array_map(function($part) { return 'unifill_b.' . $part; }, $this->part_fields()));
		...
		if (!empty($request->date_from)) { $db->where('unifill_b.created_at', trim($request->date_from) . ' 00:00:00', '>='); }
		if (!empty($request->date_to)) { $db->where('unifill_b.created_at', trim($request->date_to) . ' 23:59:59', '<='); }
		if (!empty($request->mesin)) { $db->where('unifill_b.mesin', $request->mesin); }
		if ($fieldname) { $db->where($fieldname, $fieldvalue); }
		$db->join('mesin', 'unifill_b.mesin = mesin.id', 'LEFT')->orderBy('unifill_b.id_unifill_b', ORDER_TYPE);
		$pagination = $this->get_pagination(MAX_RECORD_COUNT); $tc = $db->withTotalCount(); $records = $db->get($this->tablename, $pagination, $fields);
		$this->view->page_title = 'Unifill'; $this->set_report_props('Unifill', 'landscape');
		return $this->render_view('unifill_b/list2.php', $this->page_data($records, $tc));
	}
				...
				if ($rec_id) {
					foreach ($this->parts as $field => $label) {
						if (!empty($_POST['kendala_' . $field])) {
							$db->insert('kendala_unifill_b', array('id_am' => $rec_id, 'mesin' => $modeldata['mesin'], 'nama_bagian' => $field, 'kendala' => $_POST['kendala_' . $field], 'kategori_tag' => $_POST['kategori_tag_' . $field], 'korelasi_tag' => $_POST['korelasi_tag_' . $field], 'klasifikasi_tag' => $_POST['klasifikasi_tag_' . $field], 'kategori_ketidaksesuaian' => $_POST['kategori_ketidaksesuaian_' . $field], 'created_at' => datetime_now()));
						}
					}
					$this->write_to_log('add', 'true'); $this->set_flash_msg('Berhasil tambah AM Unifill', 'success'); return $this->redirect('unifill_b');
				}
				$this->set_page_error();
			}
		}
		$this->view->page_title = 'Add New AM Unifill'; return $this->render_view('unifill_b/add.php', array('parts' => $this->parts));
	}

	function view($rec_id = null, $value = null)
	{
		$db = $this->GetModel(); $this->rec_id = $rec_id; $fields = array_merge(array('unifill_b.id_unifill_b', 'unifill_b.mesin', 'mesin.nama_mesin AS nm_mesin', 'unifill_b.created_at', 'unifill_b.user_create', 'unifill_b.user_approve', 'unifill_b.approval', 'unifill_b.tanggal_perubahan'), array_map(function($part) { return 'unifill_b.' . $part; }, $this->part_fields()));
		if ($value) { $db->where($rec_id, urldecode($value)); } else { $db->where('unifill_b.id_unifill_b', urldecode($rec_id)); }
		$record = $db->join('mesin', 'unifill_b.mesin = mesin.id', 'LEFT')->getOne($this->tablename, $fields);
		if ($record) {
			$details = $db->rawQuery('SELECT k.*, t1.kategori_tag AS teks_kategori, t2.nama AS teks_korelasi, t3.nama AS teks_klasifikasi, t4.kategori AS teks_ketidaksesuaian FROM kendala_unifill_b k LEFT JOIN tag t1 ON k.kategori_tag=t1.id LEFT JOIN korelasi t2 ON k.korelasi_tag=t2.id LEFT JOIN klasifikasi t3 ON k.klasifikasi_tag=t3.id LEFT JOIN kategori t4 ON k.kategori_ketidaksesuaian=t4.id WHERE k.id_am=?', array($record['id_unifill_b']));
		...
		$record['parts'] = $this->parts; $this->view->page_title = 'View AM Unifill'; $this->set_report_props('View AM Unifill');
		return $this->render_view('unifill_b/view.php', $record);
	}
	...
	function edit($rec_id = null, $formdata = null)
	{
		...
		if ($formdata) {
			...
			if ($this->validated()) {
				$db->where('id_unifill_b', $rec_id);
				$bool = $db->update($this->tablename, $modeldata);
				$numRows = $db->getRowCount();
				if ($bool && $numRows) { $this->write_to_log('edit', 'true'); $this->set_flash_msg('Approval berhasil diperbarui', 'success'); return $this->redirect('unifill_b'); }
				if ($db->getLastError()) {
					$this->set_page_error();
				} elseif (!$numRows) {
					$page_error = 'No record updated';
					$this->set_page_error($page_error); $this->set_flash_msg($page_error, 'warning'); return $this->redirect('unifill_b');
				}
			}
		}
		$db->where('id_unifill_b', $rec_id); $data = $db->getOne($this->tablename, array('id_unifill_b', 'approval'));
		$this->view->page_title = 'Approve AM Unifill'; return $this->render_view('unifill_b/edit.php', $data);
	}
		...
		if ($formdata) {
			...
			if ($this->validated()) {
				...
				if ($bool) {
					...
					$db->delete('kendala_unifill_b');
					foreach ($this->parts as $field => $label) {
						if (!empty($_POST['kendala_' . $field])) {
							$db->insert('kendala_unifill_b', array('id_am' => $rec_id, 'mesin' => $mesin_id, 'nama_bagian' => $field, 'kendala' => $_POST['kendala_' . $field], 'kategori_tag' => $_POST['kategori_tag_' . $field], 'korelasi_tag' => $_POST['korelasi_tag_' . $field], 'klasifikasi_tag' => $_POST['klasifikasi_tag_' . $field], 'kategori_ketidaksesuaian' => $_POST['kategori_ketidaksesuaian_' . $field], 'created_at' => datetime_now()));
						}
					}
					$this->write_to_log('edit_data', 'true');
					$this->set_flash_msg('Data berhasil diperbarui', 'success');
					return $this->redirect('unifill_b/view/' . $rec_id);
				}
				if ($db->getLastError()) {
					$this->set_page_error();
				} elseif (!$numRows) {
					$page_error = 'Tidak ada perubahan data yang disimpan';
					$this->set_page_error($page_error); $this->set_flash_msg($page_error, 'warning'); return $this->redirect('unifill_b/view/' . $rec_id);
				}
			}
		}
		$db->where('unifill_b.id_unifill_b', $rec_id);
		$record = $db->join('mesin', 'unifill_b.mesin = mesin.id', 'LEFT')->getOne($this->tablename, array('unifill_b.*', 'mesin.nama_mesin AS nm_mesin'));
		if ($record) {
			$details = $db->rawQuery('SELECT k.* FROM kendala_unifill_b k WHERE k.id_am=?', array($rec_id));
			$record['abnormalitas'] = array(); foreach ($details as $detail) { $record['abnormalitas'][$detail['nama_bagian']] = $detail; }
		} else {
			$this->set_page_error($db->getLastError() ?: 'No record found'); $record = array();
		}
		$record['parts'] = $this->parts; $this->view->page_title = 'Edit Data AM Unifill';
		return $this->render_view('unifill_b/edit_data.php', $record);
	}

	function editfield($rec_id = null, $formdata = null)
	{
		$db = $this->GetModel(); $this->rec_id = $rec_id;
		$this->fields = array('id_unifill_b', 'updated_at', 'approval', 'user_approve', 'perubahan', 'user_perubah', 'tanggal_perubahan');
		$page_error = null;
		if ($formdata) {
			$postdata = array();
	...
	function delete($rec_id = null) { Csrf::cross_check(); $db = $this->GetModel(); $this->rec_id = $rec_id; $ids = array_map('trim', explode(',', $rec_id)); $db->where('id_unifill_b', $ids, 'in'); if ($db->delete($this->tablename)) { $this->write_to_log('delete', 'true'); $this->set_flash_msg('Record deleted successfully', 'success'); } else { $this->set_flash_msg($db->getLastError(), 'danger'); } return $this->redirect('unifill_b'); }
}
