...
$part_rows = $master_db->GetModel()->where('machine_key', 'storage_tank_tetrapak')->where('taken_out_at', null, 'IS')->orderBy('urutan', 'ASC')->get('master_part');
$part_details = array();
$sections = array();
...
$page_element_id = 'storage_tank_tetrapak-add-' . random_str();
?>
<section class="page" id="<?php echo $page_element_id; ?>">
  <div class="bg-light p-3 mb-3">
    <div class="container-fluid">
      <h4 class="record-title">Add Autonomous Maintenance Storage Tank Tetrapak</h4>
...
<?php } ?>
          <form id="storage_tank_tetrapak-add-form" class="form page-form needs-validation" novalidate
            action="<?php print_link("storage_tank_tetrapak/add?csrf_token=$csrf_token") ?>" method="post" enctype="multipart/form-data"><?php if (!empty($is_shift_form) && !empty($selected_shift)) { ?><input type="hidden" name="shift" value="<?php echo htmlspecialchars($selected_shift); ?>"><?php } ?>
  ...
  if (empty($visible_fields)) { continue; } ?>
            ...
            <div class="text-center mt-4 mb-4 pt-2 pb-2"><a class="btn btn-secondary mr-3" href="<?php print_link('storage_tank_tetrapak') ?>">Batal</a><button
