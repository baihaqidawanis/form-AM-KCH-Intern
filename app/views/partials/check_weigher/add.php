...
// Unit fisik Check Weigher (Check Weigher Jinsung 1..5)
$unit_options = $model->GetModel()->where('nama_mesin', 'Check Weigher%', 'LIKE')->orderBy('nama_mesin', 'ASC')->get('mesin');
...
$page_element_id = 'check_weigher-add-' . random_str();
?>
<section class="page" id="<?php echo $page_element_id; ?>">
  <div class="bg-light p-3 mb-3">
    <div class="container-fluid">
      <h4 class="record-title">Add Autonomous Maintenance Check Weigher</h4>
...
<?php } ?>
          <form id="check_weigher-add-form" class="form page-form needs-validation" novalidate
            action="<?php print_link("check_weigher/add?csrf_token=$csrf_token") ?>" method="post" enctype="multipart/form-data"><?php if (!empty($is_shift_form) && !empty($selected_shift)) { ?><input type="hidden" name="shift" value="<?php echo htmlspecialchars($selected_shift); ?>"><?php } ?>
  ...
  if (empty($visible_fields)) { continue; } ?>
            ...
            <div class="text-center mt-4 mb-4 pt-2 pb-2"><a class="btn btn-secondary mr-3" href="<?php print_link('check_weigher') ?>">Batal</a><button
