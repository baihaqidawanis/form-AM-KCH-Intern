...
?>
<section class="page"><div class="bg-light p-3 mb-3"><div class="container"><h4>View AM Unifill B</h4></div></div><div class="container"><?php $this::display_page_errors(); ?>
<?php if (!empty($data['id_unifill_b'])) { ?>
<div class="card">
  <div class="card-body">
    <div id="page-report-body">
      <table class="table table-bordered"><tr><th width="25%">Nama Mesin</th><td><?php echo $data['nm_mesin'] ?: '-'; ?></td></tr><tr><th>Tanggal</th><td><?php echo format_am_date($data["created_at"]); ?></td></tr><tr><th>Pembuat</th><td><?php echo $data['user_create']; ?></td></tr><tr><th>User Approve</th><td><?php echo $data['user_approve'] ?: '-'; ?></td></tr><tr><th>Approval</th><td><?php echo $data['approval'] ?: '-'; ?></td></tr><tr><th>Tanggal Approve</th><td><?php echo $data['tanggal_perubahan'] ? format_am_date($data['tanggal_perubahan']) : '-'; ?></td></tr><tr><th>User Update</th><td><?php echo $data['user_perubah'] ?: '-'; ?></td></tr><tr><th>Tanggal Update</th><td><?php echo $data['updated_at'] ? format_am_date($data['updated_at']) : '-'; ?></td></tr><tr><th>Perubahan</th><td><?php echo nl2br(htmlspecialchars($data['perubahan'] ?: '-')); ?></td></tr></table>
      <div class="table-responsive">
        <table class="table table-bordered">
          <thead><tr><th>Nama Part</th><th>Kondisi</th><th>Kendala</th><th>Kategori Tag</th><th>Korelasi</th><th>Klasifikasi</th><th>Ketidaksesuaian</th><th>Foto Before</th></tr></thead>
          <tbody>
            <?php foreach ($parts as $field => $label) { $abn = (($data[$field] ?? '') === 'NOK') ? ($data['abnormalitas'][$field] ?? null) : null; ?>
              <tr>
                ...
                <td><?php echo $abn['teks_ketidaksesuaian'] ?? '-'; ?></td>
                    <?php include dirname(__DIR__) . '/machine_nok_evidence_view.php'; ?>
              </tr>
            <?php } ?>
          </tbody>
        </table>
    ...
    <div class="mt-3 d-flex flex-wrap align-items-center">
      <a class="btn btn-secondary mx-1" href="<?php print_link('unifill_b') ?>"><i class="fa fa-arrow-left"></i> Kembali</a>
      ...
      <?php if ($can_approve) { ?>
        <a class="btn btn-sm btn-info has-tooltip mx-1" title="Approve This Record" href="<?php print_link("unifill_b/edit/$rec_id"); ?>">
          <i class="fa fa-check-circle"></i> Approval
        </a>
      <?php } ?>
      <?php if ($can_edit) { ?>
        <a class="btn btn-sm btn-warning mx-1" href="<?php print_link("unifill_b/edit_data/$rec_id"); ?>">
          <i class="fa fa-edit"></i> Edit Data
        </a>
      <?php } ?>
      <?php if ($can_delete) { ?>
        <a class="btn btn-sm btn-danger record-delete-btn mx-1"
          href="<?php print_link("unifill_b/delete/$rec_id/?csrf_token=$csrf_token&redirect=$current_page"); ?>"
