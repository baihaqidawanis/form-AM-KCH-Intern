...
$data = $this->view_data;
$parts = $data['parts'];
$rec_id = !empty($data['id_granulator']) ? $data['id_granulator'] : null;
$csrf_token = Csrf::$token;
$current_page = $this->set_current_page_link();
?>
<section class="page">
  <div class="bg-light p-3 mb-3"><div class="container"><h4>View AM Granulator</h4></div></div>
  <div class="container">
    <?php $this::display_page_errors(); ?>
    <?php if (!empty($data['id_granulator'])) { ?>
    <div class="card">
      <div class="card-body">
        ...
        <div class="mt-3 d-flex flex-wrap align-items-center">
          <a class="btn btn-secondary mx-1" href="<?php print_link('granulator') ?>"><i class="fa fa-arrow-left"></i> Kembali</a>
          ...
          <?php if ($can_approve) { ?>
            <a class="btn btn-sm btn-info has-tooltip mx-1" title="Approve This Record" href="<?php print_link("granulator/edit/$rec_id"); ?>">
              <i class="fa fa-check-circle"></i> Approval
            </a>
          <?php } ?>
          <?php if ($can_edit) { ?>
            <a class="btn btn-sm btn-warning mx-1" href="<?php print_link("granulator/edit_data/$rec_id"); ?>">
              <i class="fa fa-edit"></i> Edit Data
            </a>
          <?php } ?>
          <?php if ($can_delete) { ?>
            <a class="btn btn-sm btn-danger record-delete-btn mx-1"
              href="<?php print_link("granulator/delete/$rec_id/?csrf_token=$csrf_token&redirect=$current_page"); ?>"
