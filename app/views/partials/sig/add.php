...
$part_rows = $master_db->GetModel()->where('machine_key', 'sig')->where('taken_out_at', null, 'IS')->orderBy('urutan', 'ASC')->get('master_part');
$part_details = array();
$sections = array();
...
<?php } ?>
          <form id="sig-add-form" class="form page-form needs-validation" novalidate
            action="<?php print_link("sig/add?csrf_token=$csrf_token") ?>" method="post"><?php if (!empty($is_shift_form) && !empty($selected_shift)) { ?><input type="hidden" name="shift" value="<?php echo htmlspecialchars($selected_shift); ?>"><?php } ?>
  ...
  if (empty($visible_fields)) { continue; } ?>
              <div class="section-block mb-4">
                ...
                <?php foreach ($section_fields as $field) {
                  ...
                  $kondisi_options = ($field === 'antistatic') ? Menu::$antistatic : Menu::kondisi_options($info['highlight']);
